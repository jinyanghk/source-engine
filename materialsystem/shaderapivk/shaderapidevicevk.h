...
{
public:
	CShaderDeviceVK() : m_DynamicMesh( true ), m_Mesh( false ) {}
...
private:
	CVKMesh m_Mesh;
	CVKMesh m_DynamicMesh;
};

static CShaderDeviceVK s_ShaderDeviceGL;

// FIXME: Remove; it's for backward compat with the materialsystem only for now
EXPOSE_SINGLE_INTERFACE_GLOBALVAR( CShaderDeviceVK, IShaderDevice, 
