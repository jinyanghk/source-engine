...
bool CHammerApp::Create()
{
    ...
    // --- PURE ZERO-HARDCODING SHADER BINDING GATE ---
    // Extract the -game value early right here during creation to lock down the true shader binary
    const char *pGameDirParam = CommandLine()->ParmValue("-game", "hl2");
    std::string creationModName = pGameDirParam;

    printf("[DEBUG] Creating material system shader bindings for game module: '%s'\n", creationModName.c_str());

    // Instruct the Material System to look up the exact, native hardware shader API
    // standardly compiled inside your waf build environment prefix folder paths
...
bool CHammerApp::PreInit()
{
    ...
    // Query the raw OS filesystem directly using fopen
    ...
    // Force add the calculated path folder location into the engine's tracking registry array
    g_pFileSystem->AddSearchPath(initInfo.m_pDirectoryName, "GAME");

    // --- CRITICAL ARCHITECTURAL FIX: INJECT FOUNDATIONAL ASSETS PATH ---
    // If running a mod like portal, this lets the engine search inside the base hl2/
    // folder for core editor textures and standard UI font sheets cleanly!
...
int CHammerApp::Main()
{
    ...
    // Start with a clean, un-rendered asset canvas placeholder
    g_CurrentModelPath = "";
