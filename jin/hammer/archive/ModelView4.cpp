...
QModelView4::QModelView4(QWidget *parent)
...
void QModelView4::updateBrushes(const MapBrush *pBrushes, int count, int selectedId)
{
    m_mapBrushes.clear();
...
void QModelView4::LoadModelFile(const QString &szPath)
{
    if (!g_pMDLCache)
        return;
...
void QModelView4::RenderEngineFrame()
{
...
void QModelView4::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event);
    RenderEngineFrame();
    ...
    {float rotatedX = x * qCos(radY) - y * qSin(radY);float rotatedY = x * qSin(radY) + y * qCos(radY);float finalX = rotatedX;float finalY = rotatedY * qCos(radX) - z * qSin(radX);float sX = (w / 2.0f) + (finalX * m_flZoomScale * 1.8f) + m_ptCameraPanOffset.x();float sY = (h / 2.0f) - (finalY * m_flZoomScale * 1.8f) + m_ptCameraPanOffset.y();return QPointF(sX, sY); };
    if (m_hCurrentModel != 0xFFFF && g_pMDLCache)
    {
        ...
        if (pStudioHdr && pHardwareData && pHardwareData->m_NumLODs > 0 && pHardwareData->m_pLODs != nullptr)
        {
            ...
            // Draw skeleton nodes overlay layer
            for (int i = 0; i < pStudioHdr->numbones; ++i)
            {
                QPointF p1 = Project3DPointEx(pBoneToWorld[i].m_flMatVal[0][3], pBoneToWorld[i].m_flMatVal[1][3], pBoneToWorld[i].m_flMatVal[2][3]);
                ...
                {
                    QPointF p2 = Project3DPointEx(pBoneToWorld[parentIdx].m_flMatVal[0][3], pBoneToWorld[parentIdx].m_flMatVal[1][3], pBoneToWorld[parentIdx].m_flMatVal[2][3]);
...
void QModelView4::mousePressEvent(QMouseEvent *event) { m_ptLastMousePosition = event->pos(); }
void QModelView4::mouseMoveEvent(QMouseEvent *event)
{
...
void QModelView4::wheelEvent(QWheelEvent *event)
{
    m_flZoomScale += event->angleDelta().y() > 0 ? 0.1f : -0.1f;
    m_flZoomScale = qBound(0.1f, m_flZoomScale, 10.0f);
    this->update();
}
void QModelView4::resizeEvent(QResizeEvent *event) { QWidget::resizeEvent(event); }
int QModelView4::GetSequenceCount() { return 0; }
const char *QModelView4::GetSequenceName(int index)
{
    Q_UNUSED(index);
    return "";
}
void QModelView4::SetActiveSequence(int index) { Q_UNUSED(index); }
void QModelView4::SetAnimationCycle(float flCycle) { Q_UNUSED(flCycle); }
void QModelView4::SetPlaybackPaused(bool bPaused) { Q_UNUSED(bPaused); }